...
The mosaicplot shows that the largest group of birds seen is "None" followed by "2-10" and "11-100". So **most often we do not see a birds before a strike** happens and most often 1 bird is struck. But if we see something, than according to the data we only see birds in groups, where larger groups of birds are less likely than smaller ones. If the pilot sees birds, **quite often less birds were hit than seen**.  Interesting here is, that if the pilot sees 2-10 birds, he hits about 40% of the time less birds and for the group birds seen = 11-100, about 85% of the time the pilot hits less birds than seen. The event of hitting less birds than seen will later be analyzed more in detail.
...
The top plot shows a boxplot of height for flight phases, which are above ground and on the bottom we will have a closer look on the height for the flight phases "En Route" and "Descent", because they show the most variability in height. We defined Climb, En Route, Descent, Approach as phases which are above ground level.
...
### 3.1.2 density birdsstuck by height for flight phases above ground 
...
Time of Day: Based on the assumption that missing values in the data for birds_seen stands for the fact that the pilot did not see wildlife before a strike happened, the proportion of birds seen before a strike happened is 0.217+0.069=0.286. The contingency table looks like the following: 
...
less_strike_sums <- rep(addmargins(table(seen_vs_strike$less_strike_than_seen, seen_vs_strike$phase_of_flt),1)[3,],each=2)
...
seen_vs_strike_phase <- ggplot(seen_vs_strike[!is.na(seen_vs_strike$phase_of_flt),]) +
 ...
 # geom_text(aes(label=..count..),
  geom_text(aes(label=ifelse(round(..count../less_strike_sums[2:16],2) > 0.1,round(..count../less_strike_sums[2:16],2) , "")),
            stat='count',
            size = 3,
            position=position_fill(vjust = 0.5))+
  coord_flip() +
