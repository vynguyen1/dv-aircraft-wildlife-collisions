...
color_pal_TOD<-c("paleturquoise","lightgoldenrod","plum2","midnightblue")
theme.title<-theme(
...
top20_plot<-ggplot(as.data.frame(sort(table(birds$species), decreasing = TRUE)[1:20]), aes(y=reorder(Var1, Freq), x=Freq)) + 
  ...
  labs(title="Top 20 Most Frequent Species",
       subtitle="Wildlife that aircrafts have collided with the most",
...
cal_plot<-ggplot(as.data.frame(sort(table(california_collisions$species), decreasing = TRUE)[1:20]), aes(y=reorder(Var1, Freq), x=Freq)) + 
  geom_bar(stat="identity", fill="darkolivegreen") +
  labs(title="Top 20 in WV",
       subtitle="Most frequent Wildlife in West Virginia",
       x = "Number of Collisions",
       y = "") +
  theme.title
...
bird_wordcloud<-ggplot(is.bird.species[order(is.bird.species$Freq, decreasing = TRUE),][1:20,], aes(label = Var1, size=Freq)) +
  geom_text_wordcloud_area(area_corr_power = 1) +
  labs(subtitle="Frequent bird species") +
  theme(plot.subtitle = element_text(vjust = - 20))
  scale_size_area(max_size = 5)

non.bird.species<-as.data.frame(table(birds.with.statenames.filt[!birds.with.statenames.filt$is.bird,]$species))
non_bird_wordcloud<-ggplot(non.bird.species[order(non.bird.species$Freq, decreasing = TRUE),][1:20,], aes(label = Var1, size=Freq)) +
  geom_text_wordcloud_area(area_corr_power = 1) +
  labs(subtitle="Frequent non-bird species") +
  theme(plot.subtitle = element_text(vjust = - 20))
  scale_size_area(max_size = 5)
```
``` {r echo=FALSE}
...
half.year.plot<-ggplot(birds.filt[as.integer(birds.filt$birds_struck) > 3,]) +
  ...
  labs(x = "In the first half of the year", y = "Number of collisions") +
  guides(fill=guide_legend(title="Birds/Wildlife strucked")) +
  theme(legend.position = "none") +
  scale_fill_manual(values=c("#3182bd", "#08519c")) +
  geom_bar()
...
We again see that for almost all years there is an increase in strucked birds in the fall and less collisions in the first half of the year. The reasons for this are mentioned above. With each year the number of collisions increases. However, the result for the years "1998" and "1999" are quite low compared to the previous years. After some research there was no valid reason for this observation as normally wildlife strikes tend to increase over time which we have seen with the previous years. Therefore we assume that, at lest in this dataset, for the years "1998" and "1999" there is data missing, so not as much strikes are reported here.  
